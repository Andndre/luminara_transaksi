...
@section('content')
    ...
    <form action="{{ route('admin.invoices.store') }}" method="POST" x-data="invoiceEditor()">
        @csrf
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="customer_name" x-model="customer.name" class="w-full px-3 py-2 border rounded-lg text-sm" required>
                        </div>
                        <div>
                        ...
                        <template x-for="(item, index) in items" :key="index">
                            <div class="flex flex-col md:flex-row gap-3 p-4 bg-gray-50 rounded-xl border border-gray-200">
                                <div class="flex-1 w-full">
                                    ...
                                    <input type="text" :name="`items[${index}][description]`" x-model="item.description" class="w-full px-3 py-2 border rounded-lg text-sm" placeholder="Nama Item" required>
                                </div>
                                <div class="w-full md:w-20">
                                    <label class="text-xs text-gray-500">Qty</label>
                                    <input type="number" :name="`items[${index}][quantity]`" x-model="item.quantity" @input="calculateLine(index)" class="w-full px-3 py-2 border rounded-lg text-sm" min="1" required>
                                </div>
                                <div class="w-full md:w-32">
                                    <label class="text-xs text-gray-500">Harga</label>
                                    <input type="number" :name="`items[${index}][price]`" x-model="item.price" @input="calculateLine(index)" class="w-full px-3 py-2 border rounded-lg text-sm" min="0" required>
                                </div>
                                <div class="w-full md:w-32">
                                ...
                                <div class="flex flex-row md:flex-col items-center justify-end md:justify-center gap-4 md:gap-0 pt-2 md:pt-4 w-full md:w-auto">
                                    ...
                                    <button type="button" @click="removeItem(index)" class="text-red-500 hover:text-red-700 md:mt-2" x-show="items.length > 1">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 h-fit sticky top-4">
                ...
                <div class="mt-6 pt-6 border-t">
                    ...
                    <input type="date" name="invoice_date" value="{{ now()->format('Y-m-d') }}" class="w-full mb-3 px-3 py-2 border rounded-lg text-sm" required>
                    
                    <label class="block text-sm font-medium text-gray-700 mb-1">Jatuh Tempo</label>
                    <input type="date" name="due_date" class="w-full px-3 py-2 border rounded-lg text-sm">
                </div>
    ...
    <script>
        function invoiceEditor() {
            return {
                ...
                items: [
                    { description: '', quantity: 1, price: 0, total: 0, is_bonus: false }
                ],
                summary: {
                    subtotal: 0,
                    discount_percent: 0,
