...
@section('content')
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden max-w-4xl">
        <form action="{{ route('admin.bookings.store') }}" method="POST" class="p-8 space-y-6" x-data="bookingForm()">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2 border-b pb-4 mb-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="customer_name" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-yellow-500" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">No. WhatsApp</label>
                            <input type="text" name="customer_phone" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-yellow-500" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="md:col-span-2 border-b pb-4 mb-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="date" name="event_date" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-yellow-500" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Jam Mulai</label>
                            <input type="time" name="event_time" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-yellow-500" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Durasi (Jam)</label>
                            <input type="number" name="duration_hours" x-model="duration" @change="calculateTotal()" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-yellow-500" required min="1">
                        </div>
                        <div class="md:col-span-3">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Lokasi</label>
                            <input type="text" name="event_location" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-yellow-500">
                        </div>
                    </div>
                </div>
                        ...
                        <div>
                            ...
                            <select name="package_type" x-model="selectedPackageType" @change="calculateTotal()" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-yellow-500">
                                ...
                                @foreach($packages as $pkg)
                                    <option value="{{ $pkg->type }}">{{ $pkg->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" rows="2" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-yellow-500"></textarea>
                </div>
            </div>
