...
@section('content')
    ...
    <div class="grid grid-cols-1 gap-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between items-start gap-4">
                <div>
                ...
                <div class="flex items-center gap-2 w-full md:w-auto">
                    <a href="{{ route('admin.packages.edit', $package->id) }}" class="flex-1 md:flex-none justify-center flex items-center gap-1.5 bg-blue-50 text-blue-700 hover:bg-blue-100 px-3 py-2 rounded-xl text-xs font-bold transition shadow-sm border border-blue-100">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                        Edit
                    </a>
                    <form action="{{ route('admin.packages.destroy', $package->id) }}" method="POST" onsubmit="return confirm('Hapus paket ini?')" class="flex-1 md:flex-none">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="w-full justify-center flex items-center gap-1.5 bg-red-50 text-red-700 hover:bg-red-100 px-3 py-2 rounded-xl text-xs font-bold transition shadow-sm border border-red-100">
